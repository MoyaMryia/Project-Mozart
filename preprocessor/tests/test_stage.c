...
int main(void)
{
    ...
    float input[MOZART_INPUT_FRAME_SAMPLES] = {0};
    float output[MOZART_FRAME_SAMPLES];
    mozart_frame_meta_t meta = {0};

    if (mozart_pre_process(ctx, input, MOZART_INPUT_FRAME_SAMPLES - 1,
                           output, &meta) != -2) {
        mozart_pre_free(ctx);
    ...
    for (int frame = 0; frame < 2; frame++) {
        if (mozart_pre_process(ctx, input, MOZART_INPUT_FRAME_SAMPLES,
                               output, &meta) != 0) {
            mozart_pre_free(ctx);
            return fail("20 ms frame processing");
        }
        for (int i = 0; i < MOZART_FRAME_SAMPLES; i++) {
    ...
    for (int i = 0; i < MOZART_INPUT_FRAME_SAMPLES; i++) input[i] = 0.5f;
    ...
    float dc_rms = 0.0f;
    for (int i = 0; i < MOZART_FRAME_SAMPLES; i++) dc_rms += output[i] * output[i];
    dc_rms = sqrtf(dc_rms / MOZART_FRAME_SAMPLES);
    if (dc_rms > 1e-4f) {
        mozart_pre_free(ctx);
    ...
    float tone_rms = 0.0f;
    for (int i = 32; i < MOZART_FRAME_SAMPLES; i++)
        tone_rms += output[i] * output[i];
    tone_rms = sqrtf(tone_rms / (MOZART_FRAME_SAMPLES - 32));
