// rnnoise_init.c — 模型层初始化（从上游 rnnoise_data.c 尾部抽出）
// ============================================================================
// 原文件 78MB 全是内嵌权重数组，本项目改用外部 blob 加载（USE_WEIGHTS_FILE，
// 见 preprocessor/assets/ 与 tools/export_rnnoise_blob.c），只保留本函数。
// 权重名字与 blob 内数组一一对应。
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
