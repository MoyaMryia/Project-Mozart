...
struct tegra_xudc_iso_snapshot {
	struct tegra_xudc_ep_context context;
	struct tegra_xudc_trb ring[XUDC_TRANSFER_RING_SIZE];
	struct tegra_xudc_trb event;
	u32 regs[7];
	unsigned int ep_index, enq_ptr, deq_ptr;
	unsigned int pcs, ring_full, queued, trbs_queued;
	dma_addr_t ring_phys;
	unsigned int speed, address, interval, maxpacket;
};
...
#define XUDC_NR_EPS 32
	...
	bool pullup;
	bool async_callbacks;
	bool iso_snapshot_done;
	...
	struct work_struct iso_snapshot_work;
	struct tegra_xudc_iso_snapshot iso_snapshot;
					       ...
					       dma_addr_t addr)
{
	...
	if (addr < ep->transfer_ring_phys ||
	    addr - ep->transfer_ring_phys >=
		XUDC_TRANSFER_RING_SIZE * sizeof(*trb) ||
	    (addr - ep->transfer_ring_phys) % sizeof(*trb))
		return NULL;
...
static int ep_reload(struct tegra_xudc *xudc, unsigned int ep)
{
	xudc_writel(xudc, BIT(ep), EP_RELOAD);
	return xudc_readl_poll(xudc, EP_RELOAD, BIT(ep), 0);
}
				     ...
				     bool ioc)
{
	...
	/* Publish ownership only after the DMA consumer can see the TRB body. */
	dma_wmb();
		    ...
		    gfp_t gfp)
{
	...
	spin_lock_irqsave(&xudc->lock, flags);
	if (xudc->powergated || !ep->desc || ep->disabling ||
	    !xudc->pullup) {
		ret = -ESHUTDOWN;
		goto unlock;
	}
			...
			struct tegra_xudc_request *req)
{
	...
	if (trb_in_request(ep, req, deq_trb) && busy) {
		...
		if (req->usb_req.actual > 0) {
			...
			/* Offset into an already allocated DMA ring, not a new mapping. */
	...
	/*
	 * Also give back completed requests whose event is still pending.
	 * Giveback may free or requeue req; finish its ring updates first.
	 */
	tegra_xudc_req_done(ep, req, -ECONNRESET);
...
static int __tegra_xudc_ep_disable(struct tegra_xudc_ep *ep)
{
	...
	/* Do not free requests while an unlocked callback is still using them. */
	if (ep->givebacks || ep->disabling)
		return -EBUSY;
	...
	ret = ep_reload(xudc, ep->index);
	if (ret) {
		/* DMA ownership is unknown. Keep the queue and block endpoint reuse. */
		dev_err(xudc->dev, "EP %u disable reload timed out; retaining requests until reboot\n",
			ep->index);
		return ret;
	}
				  ...
				  const struct usb_endpoint_descriptor *desc)
{
	...
	ret = ep_reload(xudc, ep->index);
	if (ret) {
		/* Retain descriptor/ring ownership rather than recycle uncertain DMA. */
		ep->disabling = true;
		dev_err(xudc->dev, "EP %u enable reload timed out; endpoint blocked until reboot\n",
			ep->index);
	} else {
		ep_unpause(xudc, ep->index);
		ep_unhalt(xudc, ep->index);
...
static int tegra_xudc_gadget_pullup(struct usb_gadget *gadget, int is_on)
{
	...
	/* The core calls ->disconnect next; finish callbacks already in IRQ. */
	if (!is_on)
		synchronize_irq(xudc->irq);
...
static void tegra_xudc_async_callbacks(struct usb_gadget *gadget, bool enable)
{
	struct tegra_xudc *xudc = to_xudc(gadget);
	unsigned long flags;

	/* Keep port IRQs alive for the PHY's disconnect completion. */
	spin_lock_irqsave(&xudc->lock, flags);
	xudc->async_callbacks = enable;
	spin_unlock_irqrestore(&xudc->lock, flags);
}

static const struct usb_gadget_ops tegra_xudc_gadget_ops = {
	.udc_async_callbacks = tegra_xudc_async_callbacks,
					       ...
					       u16 seq_num)
{
	...
	if (!xudc->async_callbacks || !xudc->driver)
		return;
						  ...
						  struct tegra_xudc_trb *event)
{
	...
	if (!trb || trb == &ep->transfer_ring[XUDC_TRANSFER_RING_SIZE - 1]) {
		dev_err(xudc->dev, "invalid completion TRB on EP %u\n", ep->index);
		ep_halt(xudc, ep->index);
		return;
	}
	 ...
	 */
	complete = req && (short_packet || (!trb_read_chain(trb) &&
		(req->trbs_needed == req->trbs_queued)));
	residual = trb_read_transfer_len(event);
	if (complete && residual > req->usb_req.length) {
		dev_err(xudc->dev, "invalid transfer residual on EP %u\n", ep->index);
		ep_halt(xudc, ep->index);
		return;
	}

	/* Dequeued requests can still have pending events that retire ring slots. */
	if (req && short_packet)
		trb = req->last_trb;
	...
	if (complete) {
		...
		dev_warn_ratelimited(xudc->dev, "transfer event on dequeued request on EP %u\n",
				    ep->index);
	}
...
static void dump_iso_ring_state(struct tegra_xudc_ep *ep,
				struct tegra_xudc_trb *event)
{
	struct tegra_xudc *xudc = ep->xudc;
	struct tegra_xudc_iso_snapshot *snapshot = &xudc->iso_snapshot;
	struct tegra_xudc_request *req;
	const unsigned int offsets[] = { MFINDEX, EP_RELOAD, EP_PAUSE,
		EP_HALT, EP_STOPPED, EP_THREAD_ACTIVE, EP_STCHG };
	unsigned int queued = 0, trbs_queued = 0, i;

	/* One snapshot per controller lifetime, under the existing controller lock. */
	if (xudc->iso_snapshot_done) {
		ep_halt(xudc, ep->index);
		return;
	}
	xudc->iso_snapshot_done = true;

	/* DMA may still advance: this is a sampled view, not an atomic HW snapshot. */
	snapshot->event = *event;
	snapshot->ep_index = ep->index;
	snapshot->enq_ptr = ep->enq_ptr;
	snapshot->deq_ptr = ep->deq_ptr;
	snapshot->pcs = ep->pcs;
	snapshot->ring_full = ep->ring_full;
	snapshot->ring_phys = ep->transfer_ring_phys;
	snapshot->speed = xudc->gadget.speed;
	snapshot->address = ep->desc ? ep->desc->bEndpointAddress : 0;
	snapshot->interval = ep->desc ? ep->desc->bInterval : 0;
	snapshot->maxpacket = ep->usb_ep.maxpacket;
	memcpy(&snapshot->context, ep->context, sizeof(snapshot->context));
	memcpy(snapshot->ring, ep->transfer_ring, sizeof(snapshot->ring));
	for (i = 0; i < ARRAY_SIZE(offsets); i++)
		snapshot->regs[i] = xudc_readl(xudc, offsets[i]);
	list_for_each_entry(req, &ep->queue, list) {
		queued++;
		trbs_queued += req->trbs_queued;
	}
	snapshot->queued = queued;
	snapshot->trbs_queued = trbs_queued;

	/* Preserve fail-stop handling; never print the dump before requesting halt. */
	ep_halt(xudc, ep->index);
	schedule_work(&xudc->iso_snapshot_work);
}

static void tegra_xudc_iso_snapshot_work(struct work_struct *work)
{
	struct tegra_xudc *xudc = container_of(work, struct tegra_xudc,
						iso_snapshot_work);
	struct tegra_xudc_iso_snapshot *snapshot = &xudc->iso_snapshot;
	...
	dev_err(xudc->dev,
		"ISO snapshot EP %u: enq=%u deq=%u pcs=%u full=%u queued=%u submitted_trbs=%u ring_dma=%pad speed=%u addr=%#x interval=%u maxpacket=%u\n",
		snapshot->ep_index, snapshot->enq_ptr, snapshot->deq_ptr, snapshot->pcs,
		snapshot->ring_full, snapshot->queued, snapshot->trbs_queued,
		&snapshot->ring_phys, snapshot->speed, snapshot->address,
		snapshot->interval, snapshot->maxpacket);
	dev_err(xudc->dev, "ISO snapshot event: %08x %08x %08x %08x\n",
		le32_to_cpu(snapshot->event.data_lo), le32_to_cpu(snapshot->event.data_hi),
		le32_to_cpu(snapshot->event.status), le32_to_cpu(snapshot->event.control));
	dev_err(xudc->dev, "ISO snapshot context: %08x %08x %08x %08x %08x\n",
		le32_to_cpu(snapshot->context.info0), le32_to_cpu(snapshot->context.info1),
		le32_to_cpu(snapshot->context.deq_lo), le32_to_cpu(snapshot->context.deq_hi),
		le32_to_cpu(snapshot->context.tx_info));
	for (i = 0; i < ARRAY_SIZE(snapshot->context.rsvd); i++)
		dev_err(xudc->dev, "ISO snapshot context.rsvd[%u]=%08x\n",
			i, le32_to_cpu(snapshot->context.rsvd[i]));
	for (i = 0; i < ARRAY_SIZE(offsets); i++)
		dev_err(xudc->dev, "ISO snapshot reg[%#x]=%08x\n", offsets[i], snapshot->regs[i]);
	for (i = 0; i < ARRAY_SIZE(snapshot->ring); i++)
		dev_err(xudc->dev, "ISO snapshot TRB[%u]: %08x %08x %08x %08x\n",
			i, le32_to_cpu(snapshot->ring[i].data_lo), le32_to_cpu(snapshot->ring[i].data_hi),
			le32_to_cpu(snapshot->ring[i].status), le32_to_cpu(snapshot->ring[i].control));
}
					     ...
					     struct tegra_xudc_trb *event)
{
	...
	if (ep_ctx_read_state(ep->context) == EP_STATE_DISABLED) {
		dev_warn_ratelimited(xudc->dev,
			"transfer event on disabled EP %u: code=%#x event=%08x %08x %08x %08x\n",
			ep_index, comp_code, le32_to_cpu(event->data_lo),
			le32_to_cpu(event->data_hi), le32_to_cpu(event->status),
			le32_to_cpu(event->control));
		return;
	}

	/* Ring-empty/full events need not identify a completed transfer TRB. */
	if (comp_code == TRB_CMPL_CODE_RING_UNDERRUN ||
	    comp_code == TRB_CMPL_CODE_RING_OVERRUN) {
		dump_iso_ring_state(ep, event);
		dev_err(xudc->dev, "ISO ring error %#x on EP %u; endpoint stopped\n",
			comp_code, ep_index);
		return;
	}
...
static void tegra_xudc_port_disconnect(struct tegra_xudc *xudc)
{
	...
	if (xudc->async_callbacks && xudc->driver && xudc->driver->disconnect) {
		spin_unlock(&xudc->lock);
...
static void tegra_xudc_port_suspend(struct tegra_xudc *xudc)
{
	...
	if (xudc->async_callbacks && xudc->driver && xudc->driver->suspend) {
		spin_unlock(&xudc->lock);
...
static void tegra_xudc_port_resume(struct tegra_xudc *xudc)
{
	...
	if (xudc->async_callbacks && xudc->driver && xudc->driver->resume) {
		spin_unlock(&xudc->lock);
...
static void tegra_xudc_process_event_ring(struct tegra_xudc *xudc)
{
	...
	while (true) {
		...
		if ((le32_to_cpu(READ_ONCE(event->control)) & 1) != xudc->ccs)
			break;

		/* The device publishes the event body before handing over its cycle. */
		dma_rmb();
...
static int tegra_xudc_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&xudc->iso_snapshot_work, tegra_xudc_iso_snapshot_work);
...
free_eps:
	/* Block IRQ-side snapshot publication before draining the worker. */
	spin_lock_irq(&xudc->lock);
	xudc->iso_snapshot_done = true;
	spin_unlock_irq(&xudc->lock);
	cancel_work_sync(&xudc->iso_snapshot_work);
