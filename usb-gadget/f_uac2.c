...
afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
{
	...
	if (READ_ONCE(uac2_quarantined) || agdev->quarantined)
		return -ESHUTDOWN;
...
afunc_set_alt(struct usb_function *fn, unsigned intf, unsigned alt)
{
	...
	if (intf == uac2->ac_intf) {
		...
		if (uac2->int_ep) {
			ret = usb_ep_disable(uac2->int_ep);
			if (ret)
				return ret;
			ret = config_ep_by_speed(gadget, &agdev->func, uac2->int_ep);
			if (ret)
				return ret;
			return usb_ep_enable(uac2->int_ep);
		}
	...
	if (intf == uac2->as_out_intf) {
		...
		uac2->as_out_alt = ret ? 0 : alt;
	} else if (intf == uac2->as_in_intf) {
		if (alt)
			ret = u_audio_start_playback(&uac2->g_audio);
		else
			u_audio_stop_playback(&uac2->g_audio);
		uac2->as_in_alt = ret ? 0 : alt;
	} else {
		dev_err(dev, "%s:%d Error!\n", __func__, __LINE__);
		return -EINVAL;
	}
...
static void afunc_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (agdev->quarantined) {
		WRITE_ONCE(uac2_quarantined, true);
