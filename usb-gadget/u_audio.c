...
static void u_audio_iso_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	/* The stop path owns requests, including failed requeues. */
	if (!READ_ONCE(prm->ep_enabled))
		return;

	/* Do not requeue into a UDC nuke loop after a transport error. */
	if (status) {
		dev_err_ratelimited(uac->card->dev, "audio completion failed: %d\n", status);
		return;
	}
				       ...
				       struct usb_request *req)
{
	...
	/* i/f shutting down */
	if (!READ_ONCE(prm->fb_ep_enabled))
		return;

	if (status) {
		dev_err_ratelimited(uac->card->dev, "feedback completion failed: %d\n", status);
		return;
	}
...
static inline int free_ep(struct uac_rtd_params *prm, struct usb_ep *ep)
{
	struct snd_uac_chip *uac = prm->uac;
	struct g_audio *audio_dev;
	struct uac_params *params;
	...
	WRITE_ONCE(prm->ep_enabled, false);
	/* Disable drains queued callbacks before we release any allocations. */
	ret = usb_ep_disable(ep);
	if (ret) {
		dev_err(uac->card->dev, "%s: disable failed; retaining requests\n",
			__func__);
		return ret;
	}
...
static inline int free_ep_fback(struct uac_rtd_params *prm, struct usb_ep *ep)
{
	struct snd_uac_chip *uac = prm->uac;
	int ret;
	...
	WRITE_ONCE(prm->fb_ep_enabled, false);
	ret = usb_ep_disable(ep);
	if (ret) {
		dev_err(uac->card->dev, "%s: disable failed; retaining requests\n",
			__func__);
...
int u_audio_start_capture(struct g_audio *audio_dev)
{
	...
	WRITE_ONCE(prm->ep_enabled, false);
	WRITE_ONCE(prm->fb_ep_enabled, false);
	ret = free_ep_fback(prm, ep_fback);
	cleanup_ret = free_ep(prm, ep);
	if (ret)
		return ret;
	if (cleanup_ret)
		return cleanup_ret;
	...
	if (ret)
		return ret;
	WRITE_ONCE(prm->ep_enabled, true);
	...
	for (i = 0; i < params->req_number; i++) {
		...
		ret = usb_ep_queue(ep, prm->reqs[i], GFP_ATOMIC);
		if (ret)
			goto fail;
	}
	...
	if (ret)
		goto fail;
	WRITE_ONCE(prm->fb_ep_enabled, true);
	...
	ret = usb_ep_queue(ep_fback, req_fback, GFP_ATOMIC);
	if (ret)
		goto fail;
					...
					const char *card_name)
{
	...
	if (!g_audio)
		return -EINVAL;
	if (g_audio->quarantined || g_audio->uac)
		return -EBUSY;
...
void g_audio_cleanup(struct g_audio *g_audio)
{
	struct snd_uac_chip *uac;
	struct snd_card *card;
	int feedback_ret, capture_ret, playback_ret;
	...
	uac = g_audio->uac;
	card = uac->card;
	if (g_audio->quarantined)
		return;
	if (card)
		snd_card_disconnect(card);
	WRITE_ONCE(uac->c_prm.ep_enabled, false);
	WRITE_ONCE(uac->c_prm.fb_ep_enabled, false);
	WRITE_ONCE(uac->p_prm.ep_enabled, false);
	feedback_ret = free_ep_fback(&uac->c_prm, g_audio->in_ep_fback);
	capture_ret = free_ep(&uac->c_prm, g_audio->out_ep);
	playback_ret = free_ep(&uac->p_prm, g_audio->in_ep);
	if (feedback_ret || capture_ret || playback_ret) {
		/* Unbind cannot return an error. Keep callback code and parents alive. */
		g_audio->quarantined = true;
		__module_get(THIS_MODULE);
		dev_err(&g_audio->gadget->dev,
			"audio teardown failed; instance quarantined until reboot\n");
		return;
	}
	if (card)
		snd_card_free(card);
