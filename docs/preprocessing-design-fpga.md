...
> 相关文档：
> - `docs/preprocessing-design.md` — 纯 Jetson 现阶段版
> FPGA↔Jetson 硬件层接口规格见附录 D；预处理→后处理接口规格见 `docs/preprocessing-design.md §4`。
...
管线模块 ⑧ VAD/打标 产出 PTS / 帧号 / VAD 标志 / 能量 / 去噪置信度 / 段编号 等元数据，附在每帧上送往后处理。**精确字节布局随契约冻结再定**（见 §10）——后处理阶段尚未设计，过早定 12 字节格式属于过度规范。

### 4.3 传输形态
- 进程内：`FrameBuf { samples: Box<[f32;320]>, meta: FrameMeta }` 走 mpmc 无锁队列
- 跨进程/跨语言：定长包（魔数 + meta + PCM），走 shared ring 或 PipeWire 虚流。具体长度待契约冻结后定。
...
- **FPGA 能力边界（明确划死）**：FPGA 只做"近声源、定点、可流水线化的小模型"——RNNoise / 波束 / 轻量 AEC / AGC 这类。**人声识别类大模型（ASR / 说话人 / 情感 / 翻译 / LLM）一律留 Jetson**，不动 FPGA。两条硬理由：
  1. **物理装不下**：XC7Z020 PL BRAM 仅 630 Kb ≈ 78 KB，INT8 量化后的 Small Conformer / Whisper-tiny 至少 5–10 MB 权重，差两个数量级；只能用 PS DDR，违背"PS 仅做 boot"约束。
  2. **架构不匹配**：大模型是变长、自回归、含 attention/控制流的推理，PL 流水线化收益骤降；Jetson Ampere GPU + TensorRT 才是这类模型的本职载体。
  - 这条边界也意味着 FPGA 版的 §B.3 `DenoiseBackend` trait 中只有 RNNoise 在 PL，其他后处理能力永远在 Jetson——这是异构分工的正确形态，不是妥协。
...
> 最终版 dtbo 以附录 D.5 为准。本小节仅保留实验记录。
...
## 附录 D：FPGA ↔ Jetson 接口规格（汇总）

> 物理/链路层契约。Jetson 一侧只看见一张声卡；FPGA 一侧只看见 4 根 I²S + GND/电源。两边没有运行时控制通道。

### D.1 接口定位

只承载一件事：**FPGA 把已降噪的 PCM 流以 I²S master 身份推给 Jetson**。所有"识别"类语义一律在 Jetson 后处理做（见 `docs/preprocessing-design.md §4`）。

不做的事：
- 不传元数据（VAD/segment_id/conf 全部在 Jetson 端重算）
- 不传配置/命令（bitstream 内固化，0 运行时控制）
- 不暴露 PCIe / 不挂门铃
- 不要求 MCLK 出板（see §B.8.4）

### D.2 物理层

| 信号 | 方向 | FPGA 侧 (EBF ZYNQ7020 PL Bank 34, LVCMOS33) | Jetson 侧 (40-pin) | SoC pad |
|---|---|---|---|---|
| BCLK | FPGA → Jetson | R19 | pin 35 | soc_gpio44_pi2 |
| LRCLK / FSYNC | FPGA → Jetson | V12 (AUDIO_ADCLRC) | pin 12 | soc_gpio41_ph7 |
| DIN (audio → Jetson) | FPGA → Jetson | P19 (AUDIO_ADCDAT) | pin 38 | soc_gpio43_pi1 |
| DOUT (AEC 远端参考,可选) | Jetson → FPGA | V15 (AUDIO_DACDAT) | pin 40 | soc_gpio42_pi0 |
| MCLK | FPGA 自用 (喂 WM8960) | W15 | — | — |
| GND | | 底板任 GND | pin 9 / 25 / 39 | — |

电平对照：Bank 34 HR LVCMOS33 ↔ Jetson 40-pin 3.3V，**直连兼容，免电平转换**。实验室飞线 OK；产品化需转接板控 BCLK 3.072 MHz 阻抗与等长。

### D.3 时钟

- **FPGA = I²S master**：BCLK / LRCLK 由 FPGA 板 TCXO 12.288 MHz + PL MMCM 生成。
- **Jetson I²S2 = slave**：device-tree 中 `hdr40_snd_i2s_dap_ep` 节点写 `bitclock-slave; frame-slave;`（见 §D.5）。
- **ASRC 兜底**：Jetson APE 的 ASRC 把 FPGA 时钟漂移到 48k 严格量子上。

### D.4 音频格式与时序

| 项 | 值 | 备注 |
|---|---|---|
| 采样率 | 48 kHz | FPGA 端 codec 与 Jetson PipeWire 统一在 48k |
| 位宽 | 24-bit | I²S 标准；ALSA 收 32-bit container，高 24 位有效 |
| 声道 | stereo (标准 LRCLK 左/右) | 单麦时可左声道冗余/右声道静音 |
| 帧大小 | 480 samples (10 ms) | 与 PipeWire quantum=480 对齐 |
| I²S 模式 | Standard Philips I²S | 左声道 LRCLK=低，右声道=高 |
| BCLK | 3.072 MHz = 48k × 64 bit | 下降沿变化，上升沿采样 |
| 算法延迟 | V1 bitstream ~20 ms | 10 ms 帧 + ~10 ms RNNoise 量化窗 |

### D.5 Jetson 侧 device-tree 配置

挂载点：`&hdr40_snd_i2s_dap_ep`（base dtb 别名 `tegra_i2s2`，已 `status="okay"`）。
...
/ {
    ...
    fragment@0 {
        ...
        __overlay__ {
            ...
            pinctrl-0 = <&i2s2_slave_pins>;
            i2s2_slave_pins: i2s2_slave_grpf {
                hdr40-pin12 { nvidia,pins = "soc_gpio41_ph7"; nvidia,function = "i2s2"; nvidia,pull = <1>; nvidia,tristate = <0>; nvidia,enable-input = <1>; };
                hdr40-pin35 { nvidia,pins = "soc_gpio44_pi2"; nvidia,function = "i2s2"; nvidia,pull = <1>; nvidia,tristate = <0>; nvidia,enable-input = <1>; };
                hdr40-pin38 { nvidia,pins = "soc_gpio43_pi1"; nvidia,function = "i2s2"; nvidia,pull = <1>; nvidia,tristate = <1>; nvidia,enable-input = <1>; };
                hdr40-pin40 { nvidia,pins = "soc_gpio42_pi0"; nvidia,function = "i2s2"; nvidia,pull = <1>; nvidia,tristate = <0>; nvidia,enable-input = <0>; };
            };
        };
    };
...
编译加载：`dtc -I dts -O dtb -o mozart-i2s2-slave.dtbo mozart-i2s2-slave.dts` → 放到 `/boot/` → `extlinux.conf` 追加 `FDT mozart-i2s2-slave.dtbo`。

### D.6 上电序列

```
T0  Jetson 上电启动，kernel 加载 dtbo: I²S2 配为 slave
T1  PipeWire 起来，等待 source
─────────────────────────────────────
T2  ZYNQ 加电，PS BootROM 加载 bitstream 到 PL
T3  PL I²C master 写入 WM8960 寄存器表（~100 ms）
T4  PL I²S master 开始从 WM8960 收 PCM → 推给 Jetson DIN
T5  ALSA 见 PCM 流到 → PipeWire 向后处理喂帧
```

T2-T4 不依赖 Jetson，纯 PL 完成。V0 bitstream 只透传（脏音频），V1 bitstream 接入 RNNoise（已降噪），对 Jetson 侧无区别。

### D.7 版本与边界

- V0：WM8960 配置 + I²S 透传，裸脏音频
- V1：+ PL 内 RNNoise pipeline，已降噪音频
- V2：+ 可选波束/AEC，仍是已降噪音频
- **元数据通道永不启用**（纯 PL 形态硬规则）
- **FPGA 端不做大模型推理**（ASR/说话人/LLM 一律 Jetson）
- **bitstream 加载完，Jetson 无法影响 FPGA 行为**